...
class yolov5_lite():
    ...
    def postprocess(self, frame, outs, pad_hw):
        ...
        for detection in outs:
            ...
            print(f"Detected class: {self.classes[classId]}, Confidence: {confidence}")  # 打印类和置信度
            if confidence > self.confThreshold and detection[4] > self.objThreshold:
                ...
                frame = self.drawPred(frame, classId, confidence, left, top, left + width, top + height)

                # 对中心点进行特殊处理或显示
                if self.classes[classId] in ["Red_Center", "Green_Center", "Blue_Center"]:
                    print(f"Drawing center point for class: {self.classes[classId]}")  # 打印确认
...
if __name__=='__main__':
    ...
    parser.add_argument('--imgpath', type=str, default='imgs/photo_2.jpg', help="image path")
    parser.add_argument('--modelpath', type=str, default='sh.onnx', help="onnx filepath")
    parser.add_argument('--classfile', type=str, default='color.names', help="classname filepath")
