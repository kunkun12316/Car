...
class Onnx:
    ...
    def process(self, input, mask, anchors):
        ...
        # 根据 input 的 shape 来推导出 grid_h 和 grid_w
        num_anchors = len(mask)  # 根据掩码数量推测 anchor 数
        grid_size = input.shape[1] // num_anchors  # grid_size 是网格的总数
        grid_h = int(np.sqrt(grid_size))  # 假设 grid 是正方形的
        grid_w = grid_h  # grid_w 与 grid_h 相同
        ...
        print(f"grid_h: {grid_h}, grid_w: {grid_w}, col size: {col.size}")

        # 将 col 和 row reshape 并重复
        col = col.reshape(grid_h, grid_w, 1, 1).repeat(3, axis=-2)
        row = row.reshape(grid_h, grid_w, 1, 1).repeat(3, axis=-2)

        # 合并 col 和 row 形成 grid
        grid = np.concatenate((col, row), axis=-1)
        # 将 grid 的维度扩展到与 box_xy 兼容的形状
        grid = np.tile(grid, (1, 11 // grid.shape[1], 1, 1))  # 根据 box_xy 的大小重复 grid
        # 确保 box_xy 和 grid 形状匹配后再进行相加
        box_xy += grid
    ...
    def onnx_detect(self, frame):
        ...
        # 假设模型只输出一个张量，则使用 outputs[0] 进行处理
        input0_data = outputs[0]

        input0_data = input0_data  # 不需要 reshape

        print(input0_data.shape)
        input_data = list()
        input_data.append(np.transpose(input0_data, (0, 2, 1)))  # 改变 [1, 25200, 11] 为 [1, 11, 25200]
