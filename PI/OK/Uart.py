...
class Uart:
	...
	def uart_send_command(self, task_id, param1, param2, wait=True, timeout=10):
		...
		if wait is True:
			...
			self.wait_for_32_ack_status(task_id,param1,param2, timeout=timeout)
		...
		# 		self.wait_for_32_ack(task_id, timeout=2)
		# 	else:
		# 		self.wait_for_32_ack(task_id, timeout=timeout)

	def wait_for_32_ack_status(self, task_id, data1 , date2 , timeout=10):
		...
		while task:  # 进入循环，直到所有确认信号接收到或超时
			if time.time() > time_end and success_status is True:  # 检查是否超时
				print("time out")
				break  # 超时跳出循环
	...
	def uart_read_move_data(self,len):
		self.uart.read(len)
